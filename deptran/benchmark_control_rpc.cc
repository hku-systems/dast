    ...
    void ClientControlServiceImpl::client_response(ClientResponse *res) {
      ...
      for (int i = 0; i < num_threads_; i++) {
        for (auto it = txn_info_[i].begin();
          ...
          res->txn_info[it->first].interval_irt_latency.insert(
                  res->txn_info[it->first].interval_irt_latency.end(),
                  it->second.interval_irt_latency.begin(),
                  it->second.interval_irt_latency.end());
          it->second.interval_irt_latency.clear();
          res->txn_info[it->first].interval_crt_latency.insert(
                    res->txn_info[it->first].interval_crt_latency.end(),
                    it->second.interval_crt_latency.begin(),
                    it->second.interval_crt_latency.end());
          it->second.interval_crt_latency.clear();
        }
      }
      ...
      pthread_rwlock_unlock(&collect_lock_);
        ...
        //switch the one in use and the one to report
      int use = 0;
      if (txn_info_switch_)
        use = 1;
      for (int i = 0; i < num_threads_; i++) {
        for (std::map<int32_t, txn_info_t>::iterator it = txn_info_[i].begin();
             it != txn_info_[i].end(); it++) {
              res->txn_info[it->first].this_irt_latency.insert(
                      res->txn_info[it->first].this_irt_latency.end(),
                      it->second.interval_irt_latencies[use]->begin(), it->second.interval_irt_latencies[use]->end());
              res->txn_info[it->first].this_crt_latency.insert(
                      res->txn_info[it->first].this_crt_latency.end(),
                      it->second.interval_crt_latencies[use]->begin(), it->second.interval_crt_latencies[use]->end());

              res->txn_info[it->first].last_irt_latency.insert(
                  res->txn_info[it->first].last_irt_latency.end(),
                  it->second.last_interval_irt_latencies[use]->begin(), it->second.last_interval_irt_latencies[use]->end());

            res->txn_info[it->first].last_crt_latency.insert(
                    res->txn_info[it->first].last_crt_latency.end(),
                    it->second.last_interval_crt_latencies[use]->begin(), it->second.last_interval_crt_latencies[use]->end());
          ...
          it->second.interval_irt_latencies[use]->clear();
          it->second.interval_crt_latencies[use]->clear();
          it->second.last_interval_irt_latencies[use]->clear();
          it->second.last_interval_crt_latencies[use]->clear();
