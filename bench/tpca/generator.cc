...
void TpcaTxnGenerator::GetTxnReq(TxnRequest *req, uint32_t cid) {
    ...
    if (fix_id_ >= 0) {
        ...
        int k1, k2, k3;
        //local ratio
        if (RandomGenerator::percentage_true(90)) {
            local = true;
        }
        while (true) {
            static auto alpha = Config::GetConfig()->coeffcient_;
            ...
            sharding_->GetTablePartitions(TPCA_BRANCH, partitions);
            req->is_local_txn_ = true;
            parid_t home_id = cid % partitions.size();
            req->home_region_id = home_id;


            if (!partitions_in_same_region((parid_t) k1 % partitions.size(), home_id) ||
                !partitions_in_same_region((parid_t) k2 % partitions.size(), home_id) ||
                !partitions_in_same_region((parid_t) k3 % partitions.size(), home_id)) {
                req->is_local_txn_ = false;
            }
            if ((parid_t) k1 % partitions.size() != home_id &&
                (parid_t) k2 % partitions.size() != home_id &&
                (parid_t) k3 % partitions.size() != home_id) {
                //should access the home par
                continue;
            }
//            if (req->is_local_txn_ == false){
//                //crt should access this shard
//                if (!partitions_in_same_region((parid_t) k1 % partitions.size(), home_id) &&
//                    !partitions_in_same_region((parid_t) k2 % partitions.size(), home_id) &&
//                    !partitions_in_same_region((parid_t) k3 % partitions.size(), home_id)) {
//                    continue;
//                }
//            }
            if (local == req->is_local_txn_) {
                break;
                //else, retry generating the txn.
            }
        }
